            ...
            if (i == 1)
            {
                ...
                if (i == 1 && j == 1)
                {
                    ScriptManager.RegisterStartupScript(this, GetType(), "toast",
                        "if(window.showAdminToast) showAdminToast('Admin Registered!','New administrator account has been created successfully.','success');", true);
                }
            }
            else
            {
                ScriptManager.RegisterStartupScript(this, GetType(), "toast",
                    "if(window.showAdminToast) showAdminToast('Registration Failed','Unable to register admin. Please try again.','error');", true);
            }
        }
    }
}